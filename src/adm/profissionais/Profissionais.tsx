...
function Profissionais() {
  ...
  useEffect(() => {
    async function fetchProfissional() {
      try {
        const response = await api.get("/administrador/profissional/api/");
  ...
  // 🔹 Abrir modal
  const abrirModalExcluir = (func: Profissional) => {
    setProfissionalSelecionado(func);
    setShowModal(true);
  };
  ...
  const excluirProfissional = async () => {
    ...
    try {
      await api.delete(`/administrador/profissional/api/${profissionalSelecionado.id}/`);
      setProfissionais((prev) => prev.filter((f) => f.id !== profissionalSelecionado.id));
      setShowModal(false);
    } catch (error) {
  ...
  return (
    <Container fluid className="py-2">
      <Card className="shadow-sm">
        <h4 className="m-2 text-center text-primary">Lista de Profissionais</h4>
                      ...
                      <tr key={func.id}>
                        ...
                        <td>
                          <div className="d-flex justify-content-center gap-2">
                            ...
                            {/* Botão Editar */}
                            <Link
                              to={`/painel/admin/profissional/editar/${func.id}`}
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body className="text-center">
          ...
          <div className="mb-2">
          <strong>{profissionalSelecionado?.nome_completo}</strong>?
          </div>
